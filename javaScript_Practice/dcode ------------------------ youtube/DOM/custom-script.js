...
/**
 * ---------------- DOM's properties , objects and functions ----------------------
 */
// document.createElement("div");
// inputEl.setAttribute("value", "burger");
// el.textContent // for getting
// el.textContent = "javascript"  // for setting
// el.innerHTML
// el.innerHTML = "<em>DOM javascript</em>"
// fruitList.appendChild(item); // fruitList is a parent element and item is a child element.
// document.createDocumentFragment(); 
// mainTitle.classList ====>prototype methods:  add(), remove(), toggle(), replace(), contains() etc
// listItem.dataset //can get and set and  will use "data-" attribute in html.
// jis_Parent_element_may_append_hoga.insertBefore(jo append hoga element , is element say pahlay );
// jo_item_clone_hona_hay.cloneNode(true);
// el.nextElementSibling; 
// vegList.outerHTML; getting
// vegList.outerHTML = "<p>This is replaced from ul</p>"; setting
// document/element.querySelector("css selector / html tag")
// event bubbling
// event capturing
// jiselementklayhaazsay.insertAdjacentHTML("beforebegin", jis element ko insert karna hay);
// jiselementklayhaazsay.insertAdjacentElement("beforebegin", jis element ko insert karna hay);
// positions of insertAdjacentHTML()
/**
 * beforebegin - insert element hoga, apnay layhaaz waly element say pahlay.
 * afterend -    insert element hoga, apnay layhaaz waly element k baad.
 * beforeend -   insert element hoga, apnay layhaaz waly element k last text child k baad.
 * afterbegin -  insert element hoga, apnay layhaaz waly element k first text child say pahlay.
 */

// Element.matches(html tags / css selector)  return true or false.
// event delegation:
// parentelement.replaceChild(newchild, ye child element replace hoga );
// parentelement.append(elemen1, element2, "a simple text");
// document.createTextNode("text content here");
// oldelement.replaceWith(newelement);
// closest("html tag / css selector ") find a parent
...
//     item1.textContent = "Mango";
//     item2.textContent = "Banana";
    
//     // appends item1, item2 into documentfragment.
//     docFrag.appendChild(item1);
//     docFrag.appendChild(item2);

//     // state of document fragment now
//     console.log( docFrag );
...
// mainTitle.classList.replace("title", "new-title");

// contains class check, return true or false
...
//         // console.log(tabsContentChildren)
...
//     onionItem.textContent = "onion piyaaz";
...
// ------------------- Element.querySelector("css selector / html tag") ---------------------------
...
// 1) event bubbling may yey hota hay k , event: child to parent ki taraf flow karta hay jub
//    hum child element par click kartay hayn.
// 2) or agar sirf parent element par click hoga toh sirf wohi occur hoga.
// 3) by default event bubbling hoti hay.
// 4) us ko stop karnay k liyey stopPropagation ka istamaal kartay hayn child par.
...
//         alert("parent element event clicked");
//     });
    
//     // child event
//     paraStrong.addEventListener("click", function() {
//         alert("child element event clicked");
//     });
...
//     // child event with stop propagation
      // is may hum nay child ko roka hay propagate honay say.
...
//     // parent event with true argument for event capturing!
//     para.addEventListener("click", function() {
//         alert("parent element event clicked");
//     }, true);
...
//  where insert positions
/**
 * beforebegin - insert element hoga, apnay layhaaz waly element pahlay.
 * afterend -    insert element hoga, apnay layhaaz waly element k baad.
 * beforeend -   insert element hoga, apnay layhaaz waly element k last text child k baad.
 * afterbegin -  insert element hoga, apnay layhaaz waly element k first text child say pahlay.
 */
...
//  where insert positions
/**
 * beforebegin - insert text hoga, apnay layhaaz waly element say pahlay.
 * afterend -    insert text hoga, apnay layhaaz waly element k baad.
 * beforeend -   insert text hoga, apnay layhaaz waly element k last text child k baad.
 * afterbegin -  insert text hoga, apnay layhaaz waly element k first text child say pahlay.
 */
...
// ---------------------------- Element.matches(html tags / css selector) -----------------------------
...
// ------------------------------------ event delegation ------------------------------------------------------------
//  we take a look at what exactly Event Delegation is and how to implement in JavaScript without
//   using a front-end library such as jQuery.
// By doing it this way, we get a better explanation of what it is and why it's useful.

// 1) a way you can add an event listener once for multiple elements with 'support' for adding extra children.
// 2) Event delegation in JavaScript is a pattern that efficiently handles events. Events can be added
//  to a parent element instead of adding to every single element.
// 3) problem: jub hum kisi aysay child par event lagatay hayn, k us jessy or aany
//  hun server say toh hota yey hy k jis par hum nay event
// lagaya hota hay us par tooh event occur hota hay, magar jo childs server k through atay hayn , un par event nai lag paata.
// hum yey chahatay hayn k jo childs server say aahayn un par b event occur ho, toh us k liyey event delegation lagatay hayn.
// is may yey hota hay k hum childs par event lagany k bajahay, un k parent par event lagatay hayn, phirh matches method k 
// zariyey childs check kar k apni jo desire interaction wo kartay hayn childs par.
...
// example:
    ...
    // // colorsList is a parent element
    // colorsList.addEventListener("click", function(e) {
    //     console.log(e); // e depends on event object that might be ul or li.
    //     console.log(this); // this will not associate with event object, it will always be DOM parent element i.e ul.
...
//     listItm = colors.children[1]; // this is second element
...
//  to replace a node with one or more other nodes.
